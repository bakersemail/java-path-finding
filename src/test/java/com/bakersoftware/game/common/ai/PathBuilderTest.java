...
public class PathBuilderTest {
	...
	public void shouldBuildPath() {
		PathBuilder<Integer> builder = new PathBuilder<Integer>(
				new EqualsComparator<Integer>());
		List<AttachedTreeNode<Integer>> path = builder.withVertex(1, 2)
				.withVertex(1, 3).withVertex(2, 3).list();
		...
		// assertEquals(path.get(0).getAdjacentNodes().get(0).getAttached(),
		// Integer.valueOf(2));
		// assertEquals(path.get(0).getAdjacentNodes().get(1).getAttached(),
		// Integer.valueOf(3));

		assertEquals(path.get(1).getAdjacentNodes().size(), 1);
		// assertEquals(path.get(1).getAdjacentNodes().get(0).getAttached(),
		// Integer.valueOf(3));
